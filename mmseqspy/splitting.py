...
from .clustering import cluster as perform_clustering  # Rename the import to avoid conflicts
...
def cluster_kfold(
    ...
    clusters_with_sizes = [(cluster_name, cluster_sizes[cluster_name]) for cluster_name in clusters_list]
...
def milp_split(
    ...
    """
    try:
        from pulp import LpProblem, LpVariable, LpMinimize, lpSum, LpStatus, LpBinary, value, PULP_CBC_CMD
    except ImportError:
        ...
        for res_col in residue_cols:
            ...
            try:
                ...
                working_df[f"{res_col}_var"] = df[res_col].apply(
                    lambda x: np.var(x) if isinstance(x, (list, np.ndarray)) and len(x) > 1 else np.nan
                )
                
                # Distribution percentiles
                working_df[f"{res_col}_p25"] = df[res_col].apply(
                    lambda x: np.percentile(x, 25) if isinstance(x, (list, np.ndarray)) and len(x) > 0 else np.nan
                )
                working_df[f"{res_col}_p50"] = df[res_col].apply(
                    lambda x: np.percentile(x, 50) if isinstance(x, (list, np.ndarray)) and len(x) > 0 else np.nan
                )
                working_df[f"{res_col}_p75"] = df[res_col].apply(
                    lambda x: np.percentile(x, 75) if isinstance(x, (list, np.ndarray)) and len(x) > 0 else np.nan
                )
                
                # Add derived columns to the list
                new_cols = [
                    f"{res_col}_mean", f"{res_col}_min", f"{res_col}_max", f"{res_col}_var",
                    f"{res_col}_p25", f"{res_col}_p50", f"{res_col}_p75"
                ]
    ...
    group_vars = {group: LpVariable(f"group_{i}", cat=LpBinary) 
    ...
    balance_vars = {col: LpVariable(f"balance_{col}", lowBound=0) for col in group_properties.keys()}
    
    # Variables to represent absolute differences for variance balances
    variance_vars = {col: LpVariable(f"variance_{col}", lowBound=0) for col in group_variances.keys()}
    
    # Variables to represent min/max range differences
    min_range_vars = {col: LpVariable(f"min_range_{col}", lowBound=0) for col in group_min_values.keys()}
    max_range_vars = {col: LpVariable(f"max_range_{col}", lowBound=0) for col in group_max_values.keys()}
    ...
    objective_terms = [
        ...
        range_weight * lpSum(list(min_range_vars.values()) + list(max_range_vars.values())) # Range balance
    ]
    ...
    for col, variance_comp in group_variances.items():
        test_variance_sum = lpSum([group_vars[group] * variance_comp[group] for group in groups 
    ...
    for col in group_min_values.keys():
        ...
        min_range_test_count = lpSum([group_vars[group] * group_min_values[col][group] for group in groups 
        ...
        max_range_test_count = lpSum([group_vars[group] * group_max_values[col][group] for group in groups 
    ...
    for col in original_cols:
        ...
        mean_diff_pct = abs(test_mean - train_mean) / (abs(train_mean) if train_mean != 0 else 1) * 100
        ...
        logger.info(f"  Mean: train={train_mean:.4f}, test={test_mean:.4f}, diff={mean_diff_pct:.2f}%")
        logger.info(f"  Variance: train={train_var:.4f}, test={test_var:.4f}, diff={var_diff_pct:.2f}%")
        logger.info(f"  Range: train=[{train_min:.4f}, {train_max:.4f}], test=[{test_min:.4f}, {test_max:.4f}]")
    ...
    if categorical_cols:
        for cat_col in categorical_cols:
            ...
            dist_diff = np.sum(np.abs(train_dist.values - test_dist.values)) / 2  # Jensen-Shannon divergence simplification
            ...
            logger.info(f"  Distribution difference: {dist_diff:.4f} (0=identical, 1=completely different)")
            ...
            if res_col in df.columns:
                ...
                for metric in ['mean', 'min', 'max', 'var', 'p25', 'p50', 'p75']:
                    ...
                    if derived_col in working_df.columns:
                        ...
                        val_diff_pct = abs(test_val - train_val) / (abs(train_val) if train_val != 0 else 1) * 100
                        
                        logger.info(f"  {metric}: train={train_val:.4f}, test={test_val:.4f}, diff={val_diff_pct:.2f}%")
