...
def test_parallel_performance():
    """Test performance with parallel processing settings."""
    # Skip if mmseqs is not installed
    import shutil

    if shutil.which("mmseqs") is None:
        pytest.skip("MMseqs2 not installed, skipping parallel performance test")
    ...
    # Test with explicit thread parameters
    import subprocess
    ...
    try:
        # Function to modify subprocess call to include thread parameter
        def run_with_threads(threads):
            def modified_run(*args, **kwargs):
                if isinstance(args[0], list) and "mmseqs" in args[0][0]:
                    # Add thread parameter
                    cmd = list(args[0])

                    # Remove any existing threads parameter
                    if "--threads" in cmd:
                        idx = cmd.index("--threads")
                        cmd.pop(idx)  # Remove --threads
                        if idx < len(cmd):
                            cmd.pop(idx)  # Remove the value

                    # Add our thread parameter
                    cmd.extend(["--threads", str(threads)])

                    # Create new args with modified command
                    new_args = (cmd,) + args[1:]
        ...
        # Test with 1 thread
        subprocess.run = run_with_threads(1)
        start_time = time.time()
        cluster(df, sequence_col="sequence", id_col="id", min_seq_id=0.8)
        single_thread_time = time.time() - start_time

        # Test with 2 threads
        subprocess.run = run_with_threads(2)
        start_time = time.time()
        cluster(df, sequence_col="sequence", id_col="id", min_seq_id=0.8)
        multi_thread_time = time.time() - start_time

        # Multi-threaded should be faster or similar
        # Allow for some overhead/variation
        assert multi_thread_time <= single_thread_time * 1.5, (
            f"Multi-threaded unexpectedly slower: {multi_thread_time:.2f}s vs {single_thread_time:.2f}s"
        )
